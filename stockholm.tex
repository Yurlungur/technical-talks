...
\date[Stockholm]{Physics Colloquium,\\ Stockholm University}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{6cm}
      \begin{center}
        \includegraphics[width=\columnwidth]{qr_nqto2}
      \end{center}
      Computationally challenging. Ask me if interested!
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  \frametitle{FFI in mergers}
  \begin{itemize}
  \item Crossings present across disk, eliminated by FFI
  \end{itemize}
  \begin{center}
    \includegraphics[width=0.9\columnwidth]{insitu/fig3_Gnu_11ms}
  \end{center}
  Lund, \textbf{JMM}, et al 2025 ApJL 985 L9
\end{frame}

\begin{frame}
  \frametitle{FFI cools disk more efficiently}
  \begin{itemize}
  ...
  \begin{center}
    ...
    \animategraphics[width=0.8\columnwidth,every=10,autoplay,loop,controls=off]
    {20}{TGK_frames/T_gk_}{0000}{1999}
  \end{center}
  Lund, \textbf{JMM}, et al 2025 ApJL 985 L9
\end{frame}

\begin{frame}
  \frametitle{FFI changes $Y_e$ and nucleosynthesis}
  \begin{columns}
    \begin{column}{4cm}
      \begin{center}
        \includegraphics[height=0.8\textheight]{insitu/fig4_Ye_PR}
      \end{center}
    \end{column}
    \begin{column}{8cm}
      \begin{itemize}
      ...
      \end{itemize}
      \begin{center}
        \includegraphics[width=0.8\columnwidth]{insitu/fig5_nucleosynthesisyields}
      \end{center}
    \end{column}
  \end{columns}
  Lund, \textbf{JMM}, et al 2025 ApJL 985 L9
...
\begin{frame}
  ...
  \item Represent 6D distribution function as \textit{tenor train} decomposition:
    $${\cal F}_{ijk\ell p q} = X_{ijk}^a (W_\theta)_{a\ell b} (W_\phi)^b_{pc} (W_\nu)^c_q$$
    \begin{itemize}
    \item $X \in \R^{N_x N_y N_z \times r_1}$ represents the spatial variables
    \item $W_\theta \in \R^{r_1 \times N_{\theta} \times r_2}$, $W_\phi \in \R^{r_2 \times N_{\phi} \times r_3}$ represent angle
    \item $W_z \in \R^{r_3 \times N_{\nu}}$ represent energy
    \end{itemize}
  \item Representation is exact for some $r$.
  \item Cost is $r_1 N_xN_yN_z + {\cal O}\left(  r^2 N \right)$ which is smaller than $N_x^6$ so a savings if $r$ small
  \item Can be worse if $r$ large
  \item Can \textit{truncate} $r$ to satisfy some error tolerance
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Tensor networks for transport}
    \begin{itemize}
    \item Potentially unparalleled angular accuracy
    \end{itemize}
  \begin{center}
    \includegraphics[height=0.8\textheight]{TTTT/f5}
  \end{center}
  {\footnotesize arXiv:2503.18056}
\end{frame}

\begin{frame}
  \frametitle{Tensor networks for transport}
  \begin{itemize}
  \item Speedup/goodness depends on chosen rounding accuracy
  \end{itemize}
  \begin{center}
    \includegraphics[height=0.8\textheight]{TTTT/f7}
  \end{center}
  {\footnotesize arXiv:2503.18056}
\end{frame}
